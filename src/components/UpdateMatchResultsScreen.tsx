...
import { ChevronLeft, Users, MapPin, Calendar, Clock, Loader2, Target, Edit } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { getMatches } from '../lib/api';
import { supabase } from '../lib/supabase';
import logo from 'figma:asset/a9109d0003972ab9d286aab63c38b1a2b2dbb9dc.png';

interface UpdateMatchResultsScreenProps {
  onBack: () => void;
  onSelectMatch: (matchId: string) => void;
}

export default function UpdateMatchResultsScreen({ onBack, onSelectMatch }: UpdateMatchResultsScreenProps) {
  const { user } = useAuth();
  ...
  const loadMatches = async (teamId?: string) => {
    ...
    try {
      // Get all matches for this team (confirmed and completed)
      const { data: matches, error } = await supabase
        ...
        `)
        .or(`team_a_id.eq.${teamIdToUse},team_b_id.eq.${teamIdToUse}`)
      ...
      // Filter matches that can be updated:
      // 1. Confirmed matches (not yet completed)
      // 2. Completed matches (can be edited)
      const updateableMatches = (matches || []).filter(match => {
        // Include confirmed matches
        if (match.status === 'confirmed') return true;
        // Include completed matches (can be edited)
        if (match.status === 'completed') return true;
        return false;
      });

      setMatchesToUpdate(updateableMatches);
    } catch (error) {
      console.error('Error loading matches:', error);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-black text-white pb-24">
        <div className="bg-gradient-to-b from-zinc-900 to-black px-6 pt-12 pb-6">
          ...
          <h1 className="text-3xl mb-2">Update <span className="text-[#00FF57]">Match Results</span></h1>
        </div>
        <div className="px-6 py-12 text-center">
          <Users className="w-16 h-16 text-zinc-700 mx-auto mb-4" />
          <p className="text-zinc-500 mb-4">You need to be a team captain to update match results</p>
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-24">
      ...
      <div className="bg-gradient-to-b from-zinc-900 to-black px-6 pt-12 pb-6">
        ...
        <h1 className="text-3xl mb-2">Update <span className="text-[#00FF57]">Match Results</span></h1>
        <p className="text-zinc-500">Select a match to update or submit results</p>
      </div>
      ...
      <div className="px-6 py-4">
        {matchesToUpdate.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-zinc-500 mb-2">No matches available to update</p>
            <p className="text-zinc-600 text-sm">Confirmed or completed matches will appear here</p>
          </div>
        ) : (
          <div className="space-y-3">
            {matchesToUpdate.map((match) => {
              ...
              const statusText = match.status === 'completed' 
                ? 'Completed' 
                : 'Confirmed';
              const hasResults = match.status === 'completed' && (match.team_a_score !== null || match.team_b_score !== null);

              return (
                <button
                  key={match.id}
                  onClick={() => onSelectMatch(match.id)}
                  className="w-full bg-gradient-to-br from-zinc-900 to-black rounded-xl p-4 border border-zinc-800 hover:border-[#00FF57]/50 transition-all active:scale-98 text-left"
                >
                  {/* Date and Status */}
                  ...
                  {hasResults && (
                    <div className="pt-3 border-t border-zinc-800">
                      ...
                      <p className="text-xs text-zinc-500 text-center mt-2">Tap to edit results</p>
                    </div>
                  )}

                  {/* Action hint for confirmed matches */}
                  {match.status === 'confirmed' && (
                    <div className="pt-3 border-t border-zinc-800">
                      <p className="text-xs text-[#00FF57] text-center">Tap to submit match results</p>
                    </div>
                  )}
                </button>
              );
